...
<section role="main" class="content-body">
    <header class="page-header">
        ...
        <div class="right-wrapper text-end">
            <ol class="custom-breadcrumbs breadcrumbs">
                ...
                <li><span>Редактирование новости</span></li>
            </ol>
        </div>
    </header>
    ...
    <section class="card">
        <header class="card-header">
            <h2 class="card-title">Новости сайта</h2>
        </header>
        <div class="card-body">
            <form class="form-horizontal form-bordered" method="POST" action="{{ route('admin.news.update', $news_item->id) }}">
                @csrf
                @method('patch')
                ...
                <div class="form-group row pb-4">
                    ...
                    <div id="editor" class="col-lg-6">
                        <textarea id="edit" name="content">{{ $news_item->content }}</textarea>
                        @error('content')
                            <span class="help-block form-input-error">Поле обязательное для заполнения.</span>
                        @enderror
                    </div>
